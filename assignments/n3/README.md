...
В Beam search для CTC: множество различных frame-level путей коллапсируют в один и тот же транскрипт (из-за blank-символов и повторений). Декодер должен отслеживать гипотезы-префиксы и суммировать вероятностности всех путей, ведущих к каждому префиксу.
...
Применяем скейлинг температуры:
```python
...
- **Rescoring** позволяет избежать этого, сохраняя акустически правдоподобные варианты, но ограничена качеством N-best списка. В данном примере rescoring улучшил результат, где shallow fusion не справилась.
